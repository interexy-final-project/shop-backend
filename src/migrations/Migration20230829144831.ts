...
export class Migration20230829144831 extends Migration {
  async up(): Promise<void> {
    this.addSql(
      'create table "products" ("id" uuid not null, "created" timestamptz(0) not null, "updated" timestamptz(0) not null, "name" text not null, "price" int not null, "images" text[] not null, "colors" text[] not null, "sizes" text[] not null, "status" text check ("status" in (\'Active\', \'Archived\')) not null default \'Active\', "description" text not null, "amount" int not null, "category" text check ("category" in (\'Woman\', \'Men\', \'Children\')) not null, "hip_girth" varchar(255) null, "sleeve_length" varchar(255) null, "waist_girth" varchar(255) null, "type" varchar(255) null, constraint "products_pkey" primary key ("id"));',
    );
    this.addSql(
    ...
    this.addSql(
      'create table "shipping_addresses" ("id" uuid not null, "created" timestamptz(0) not null, "updated" timestamptz(0) not null, "address" text not null, "city" text not null, "phone" varchar(255) not null, "user_id" uuid null, constraint "shipping_addresses_pkey" primary key ("id"));',
    );
    ...
    this.addSql(
      'create table "order_items" ("id" uuid not null, "created" timestamptz(0) not null, "updated" timestamptz(0) not null, "order_id" uuid null, "product" jsonb not null, "quantity" int not null, constraint "order_items_pkey" primary key ("id"));',
    );

    this.addSql(
      'create table "cart_items" ("id" uuid not null, "created" timestamptz(0) not null, "updated" timestamptz(0) not null, "user_id" uuid null, "product_id" uuid null, "quantity" int not null default 1, constraint "cart_items_pkey" primary key ("id"));',
    );
    this.addSql(
      'alter table "cart_items" add constraint "cart_items_product_id_unique" unique ("product_id");',
    );
  ...
  async down(): Promise<void> {
    ...
    this.addSql(
      'alter table "shipping_addresses" drop constraint "shipping_addresses_user_id_foreign";',
    );
    ...
    this.addSql(
      'alter table "cart_items" drop constraint "cart_items_user_id_foreign";',
    );
